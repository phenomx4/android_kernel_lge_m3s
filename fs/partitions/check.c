...
check_partition(struct gendisk *hd, struct block_device *bdev)
{
	...
	#if 0 // add dbg msg
	printk("check_partition kzalloc size : %d ",sizeof(struct parsed_partitions));
	show_free_areas();
	#endif 
	if (!state)
		return NULL;
	...
	printk(KERN_INFO "[check_partition] %s:", state->name);
	if (isdigit(state->name[strlen(state->name)-1]))
		sprintf(state->name, "p");

	state->limit = disk_max_parts(hd);
	printk("limit(%d):", state->limit);
...
rescan:
	...
	for (p = 1; p < state->limit; p++) {
		...
		if (from + size > get_capacity(disk)) {
			printk(KERN_WARNING
			       "%s: p%d start:%llu  size: %llu extends beyond EOD(total:%d), ",
			       disk->disk_name, p, (unsigned long long) from,(unsigned long long) size,get_capacity(disk));
