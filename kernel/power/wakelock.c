...
// matthew.choi@lge.com 120130 add delay for sys_sync [START]
#include <linux/delay.h>
// matthew.choi@lge.com 120130 add delay for sys_sync [END]
...
// matthew.choi@lge.com 120130 add delay for sys_sync [END]
extern int	suepend_work_queue_create_count;
...
//	if (debug_mask & DEBUG_SUSPEND)
	...
	ret = sys_sync();
	if (ret == 0)
		pr_info("PM: suspend_sys_sync completed...\n");
...
static void expire_wake_locks(unsigned long data)
{
	...
	if (has_lock == 0) {
		printk(KERN_INFO "[suspend] lock timer expired start suspend\n");
			...
			if (del_timer(&expire_timer))
				if (debug_mask & DEBUG_EXPIRE)
					pr_info("wake_lock: %s, stop expire timer\n",
						lock->name);
			if (expire_in == 0) {
				printk(KERN_INFO "[suspend] %s lock locked\n", lock->name);
				queue_work(suspend_work_queue, &suspend_work);
			}
		}
	}
...
void wake_unlock(struct wake_lock *lock)
{
	...
	if (type == WAKE_LOCK_SUSPEND) {
		...
		if (has_lock > 0) {
			...
			if (has_lock == 0) {
				if (lock->name[0] == 'P')
					printk(KERN_INFO "[suspend] %s lock unlocked\n", lock->name);
				queue_work(suspend_work_queue, &suspend_work);
			}
		}
		if (lock == &main_wake_lock) {
...
// matthew.choi@lge.com 120130 change create workqueue [START]
	suspend_work_queue = create_workqueue("suspend");
// matthew.choi@lge.com 120130 change create workqueue [END]
...
static int earlysuspend_stats_show(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "work queue creation : %d\n",
			suepend_work_queue_create_count);
