...
void *smem_get_entry(unsigned id, unsigned *size)
{
	...
	if (toc[id].allocated) {
		*size = toc[id].size;
		barrier();
		printk(KERN_INFO "[SMEM] allocated 0x%x id:%d",(int)(MSM_SHARED_RAM_BASE + toc[id].offset),id);
		return (void *) (MSM_SHARED_RAM_BASE + toc[id].offset);
	} else {
		printk(KERN_INFO "[SMEM] allocated fail 0x%x id:%d",toc[id].allocated,id);
...
static int __devinit msm_smd_probe(struct platform_device *pdev)
{
	...
	if (smsm_init()) {
		...
		printk(KERN_INFO "[SMEM] smsm_init() failed\n");
		return -1;
	}
	printk(KERN_INFO "[SMEM] smsm_init() success\n");

	if (smd_core_init()) {
		pr_err("smd_core_init() failed\n");
		return -1;
	}
	printk(KERN_INFO "[SMEM] smd_core_init() success\n");
