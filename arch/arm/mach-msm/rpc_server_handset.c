...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	case KEY_VOLUMEDOWN:
		...
		{
			printk(KERN_ERR, "Key lock on, END key ignored\n");
			return;
		}
		input_report_key(hs->ipdev, key, (key_code != HS_REL_K));
		if (1 == key_touch_logging_status && KEY_END == key && HS_REL_K != key_code)
		{
			mtc_send_key_log_packet((unsigned long)KEY_POWER, 1L);
		}
		break;
