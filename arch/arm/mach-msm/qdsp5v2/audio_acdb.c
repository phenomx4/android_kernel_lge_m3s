...
static s32 initialize_modem_acdb(void)
{
	...
	//20110804 hyeongwoo.seo@lge.com Debug [START]
	if(codec_type==TIMPANI_ID)
	 	printk(KERN_NOTICE "codec type TIMPANI_ID (%d)",codec_type);
	else
		printk(KERN_NOTICE "codec type MARIMBA_ID (%d)",codec_type);
	//20110804 hyeongwoo.seo@lge.com  Debug [END]
