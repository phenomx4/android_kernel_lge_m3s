...
#endif
	//ssoo.kim@lge.com 2011-12-08 : SMS Test Tool [FEATURE_SMS_PC_TEST]
	{DIAG_SMS_TEST_F, DIAG_SMS_TEST_F , LGF_SMSTest}
};
...
extern unsigned char g_diag_slate_capture_rsp_num; //Slate_ADB
...
static int diagchar_write_mtc( const char *buf, size_t count, int type)
{
	...
	printk(KERN_INFO "%s(), start, LINE:%d\n", __func__,__LINE__);
	send.pkt = NULL;
	send.last = NULL;
	send.state = DIAG_STATE_START;
	send.terminate = 0;
	...
	if (!buf_hdlc)
	{
		buf_hdlc = diagmem_alloc(driver, HDLC_OUT_BUF_SIZE,
					 POOL_TYPE_HDLC);
		printk(KERN_INFO "%s(), diagmem_alloc LINE:%d\n",__func__, __LINE__);
	}
...
*/
	...
	if ((unsigned int) enc.dest >=
		...
		buf_hdlc = diagmem_alloc(driver, HDLC_OUT_BUF_SIZE,
							 POOL_TYPE_HDLC);
		if (!buf_hdlc) {
			printk(KERN_ERR "%s(), LINE:%d diagmem_alloc failed\n",__func__, __LINE__);
			ret = -ENOMEM;
			goto fail_free_hdlc;
		}
		enc.dest = buf_hdlc + driver->used;
	...
	if (pkt_type == DATA_TYPE_RESPONSE) {
		...
		printk(KERN_ERR "%s(), LINE:%d buf_hdlc alloc success\n",__func__,__LINE__);
		driver->write_ptr_svc->buf = buf_hdlc;
		driver->write_ptr_svc->length = driver->used;
#ifdef LG_DIAG_DEBUG
...
void diagpkt_process_request (void *req_pkt, uint16 pkt_len,diag_cmd_rsp rsp_func, void *rsp_func_param)
{
	...
	printk(KERN_INFO "%s(), LINE:%d, packet_id:%d\n",__func__,__LINE__,packet_id);
	...
	printk(KERN_INFO "%s(), LINE:%d, packet_id:%d\n",__func__,__LINE__,packet_id);
	printk(KERN_INFO "%s(), LINE:%d, subsys_id:%d\n",__func__,__LINE__,subsys_id);
