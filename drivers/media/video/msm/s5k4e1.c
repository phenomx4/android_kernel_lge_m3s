...
static int32_t s5k4e1_write_exp_gain(uint16_t gain, uint32_t line)
{
	...
	CDBG("#QCT s5k4e1_write_exp_gain = %d, %d \n", gain, line);
...
// End LGE_BSP_CAMERA::tao.jin@lge.com 2012-02-13  for 15fps does not working issue (MMS Recoding Size)
		...
		CDBG("#QCT 11 s5k4e1_write_exp_gain = %d, %d \n", line, fl_lines);
...
// End LGE_BSP_CAMERA::tao.jin@lge.com 2012-02-13  for 15fps does not working issue (MMS Recoding Size)
		...
		s5k4e1_i2c_write_b_sensor(0x0202, s5k4e1_byte(line, MSB));
		s5k4e1_i2c_write_b_sensor(0x0203, s5k4e1_byte(line, LSB));
		CDBG("#QCT 33 s5k4e1_write_exp_gain = %d, %d \n", line, 0);
		s5k4e1_group_hold_off();
	}
	return rc;
}
...
// Start LGE_BSP_CAMERA::tao.jin@lge.com 2011-11-19 AF moter noise reduction
...
// End LGE_BSP_CAMERA::tao.jin@lge.com 2011-11-19 AF moter noise reduction
...
static int32_t s5k4e1_sensor_setting(int update_type, int rt)
{
	...
	if (update_type == REG_INIT) {
		...
		s5k4e1_start_stream();
		if (rt == RES_PREVIEW)
			msleep(300);//LGE_BSP_CAMERA::tao.jin@lge.com 2012-02-17 for Preview bright issue
		else
			msleep(30);//LGE_BSP_CAMERA::tao.jin@lge.com 2012-02-17 for Shutter lag spec in
	}
	return rc;
}
...
// Start LGE_BSP_CAMERA::tao.jin@lge.com 2012-02-15 Reset pin busy Err fix
/*
	...
	msleep(20);
*/
// End LGE_BSP_CAMERA::tao.jin@lge.com 2012-02-15 Reset pin busy Err fix
...
int s5k4e1_sensor_config(void __user *argp)
{
	...
	case CFG_GET_PICT_MAX_EXP_LC:
		CDBG("case CFG_GET_PICT_MAX_EXP_LC");
	...
	default:
		CDBG("default case: rc = -EFAULT");
		rc = -EFAULT;
		break;
	}
	mutex_unlock(&s5k4e1_mut);

	CDBG("<< %s END return %d", __func__, rc); 
	return rc;
}
