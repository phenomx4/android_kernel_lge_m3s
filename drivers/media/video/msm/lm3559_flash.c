	...
	unsigned char data)
{
	...
	if (client == NULL){
		printk(KERN_INFO "lm3559_i2c_write: lm3559_client is NULL\n");
		return rc;
		}

	buf[0] = addr;
	buf[1] = data;
	printk(KERN_INFO "lm3559_i2c_write addr = 0x%x, val = 0x%x\n",
		addr, data);
	...
	unsigned char *data)
{
	...
	if (client == NULL){
		printk(KERN_INFO "lm3559_i2c_read: lm3559_client is NULL\n");
		return rc;
		}
	else if (data == NULL){
		printk(KERN_INFO "lm3559_i2c_read: lm3559_data is NULL\n");
		return rc;
		}
	...
	*data = buf[0];
	printk(KERN_INFO "lm3559_i2c_read addr = 0x%x, val = 0x%x\n",
		addr, *data);
	return rc;
}
...
// Start LGE_BSP_CAMERA::tao.jin@lge.com 2011-12-07 Flash Current Changing (0110 : 393.75 mA => 0101 : 337.5 mA)
		/* 0110 : 393.75 mA */
//		lm3559_i2c_write(lm3559_client, LM3559_REG_FLASH_BRIGHTNESS,
//			0x66);
		/* 0101 : 337.5 mA */
		lm3559_i2c_write(lm3559_client, LM3559_REG_FLASH_BRIGHTNESS,
			0x55);
// End LGE_BSP_CAMERA::tao.jin@lge.com 2011-12-07 Flash Current Changing (0110 : 393.75 mA => 0101 : 337.5 mA)
	}
...
//	printk(KERN_INFO "%s: lm3559_pdata->gpio_flen[0x%x]\n",
//		__func__, lm3559_pdata->gpio_flen);
	
	printk(KERN_INFO "%s: onoff[0x%x]\n",
		__func__, onoff);
		}
	else	{
...
{
	int rc = 0;
	printk(KERN_INFO "lm3559_probe called!\n");

	if (i2c_get_clientdata(client)){
		printk(KERN_INFO "I2C add driver failed\n");
		return -EBUSY;
		}
	...
	printk(KERN_INFO "I2C add driver OK\n");
