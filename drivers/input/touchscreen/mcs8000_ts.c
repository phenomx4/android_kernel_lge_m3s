...
struct workqueue_struct *mcs8000_ts_wq = NULL;
...
printk( KERN_DEBUG "<choi> LG_FW : %s\n",__func__);
...
//		schedule_work(&dev->work);
	//	queue_delayed_work(dev->ts_wq, &dev->work,msecs_to_jiffies(TS_POLLING_TIME));
		if(mcs8000_ts_wq != NULL)
			queue_work(mcs8000_ts_wq, &dev->work);
		else
			printk( KERN_DEBUG "<choi> LG_FW : %s, mcs8000_ts_wq is NULL\n",__func__);
...
//			goto err_input_register;
	...
	*/
	...
	printk( KERN_DEBUG "<choi> LG_FW : %s,  dev->intr_gpio is %d\n",__func__, dev->num_irq);
	printk( KERN_DEBUG "<choi> LG_FW : %s,  dev->num_irq is %d , dev->intr_gpio is %d\n",__func__,dev->num_irq, dev->intr_gpio);
...
*/
...
#endif
	mcs8000_ts_wq = create_singlethread_workqueue("ts_wq");
	...
	printk( KERN_DEBUG "<choi> LG_FW : %s error is %d\n",__func__,err);
	if (err < 0) {
		printk(KERN_ERR "%s: failed to probe i2c \n", __FUNCTION__);
		goto err_i2c_add_driver;
	}
	if(mcs8000_i2c_ts_driver.probe == NULL)
		printk( KERN_DEBUG "<choi> LG_FW : %s probe is null\n",__func__);
	else
		printk( KERN_DEBUG "<choi> LG_FW : %s probe is not null\n",__func__);

	printk( KERN_DEBUG "<choi> LG_FW : %s : huhu err %d\n",__func__, err);
...
//	mcs8000_remove_file(mcs8000_ts_input);
	...
	if (mcs8000_ts_wq)
		destroy_workqueue(mcs8000_ts_wq);
