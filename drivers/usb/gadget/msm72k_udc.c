...
extern int read_cpu_performanc(const char *path);
extern int write_cpu_performanc(char * path, int on);
extern int removefile( char const *filename );

extern unsigned char g_slate_status;
extern unsigned char directory_is;
#endif
...
#endif
	printk("hyoill.leem usb_reset udc_cable : %d , manual_test mode : %d\n",udc_cable,lg_manual_test_mode);
#ifdef CONFIG_LGE_USB_GADGET_SUPPORT_FACTORY_USB
...
#ifdef CONFIG_LGE_USB_PORT_DISABLE
int user_usb_port_enable =1;
void usb_port_pullup_disable(void)
{
	struct usb_info *dev = the_usb_info;
	msm72k_pullup_internal(&dev->gadget, 0);
}
EXPORT_SYMBOL(usb_port_pullup_disable);

void usb_port_pullup_enable(void)
{
	struct usb_info *dev = the_usb_info;
	msm72k_pullup_internal(&dev->gadget, 1);
}
EXPORT_SYMBOL(usb_port_pullup_enable);
#endif
/* hyoill.leem 2012.01.16 usb port enable / disable */
...
/* hyoill.leem 2012.01.16 usb port enable / disable */
#ifdef CONFIG_LGE_USB_PORT_DISABLE
if (user_usb_port_enable == 0)
{
		usb_port_pullup_disable();
} else
#endif
/* hyoill.leem 2012.01.16 usb port enable / disable */
				msm72k_pullup_internal(&ui->gadget, 1);
...
/* hyoill.leem 2012.01.16 usb port enable / disable */
			...
			if(g_slate_status)
			{
				write_cpu_performanc(CPU_PERFORMANCE_PATH, 0);
				g_slate_status = 0x0;
			}
#endif
			break;
...
/* hyoill.leem 2012.01.16 usb port enable / disable */
#ifdef CONFIG_LGE_USB_PORT_DISABLE
int get_user_usb_port_enable(void)
{
	return user_usb_port_enable;
}
EXPORT_SYMBOL(get_user_usb_port_enable);

extern void android_usb_reset(void);
static ssize_t read_usb_port_enable(struct device *dev, struct device_attribute *attr,
				   char *buf)
{
	int ret;
	
	ret = sprintf(buf, "%d", user_usb_port_enable);
	
	return ret;
}
static ssize_t write_usb_port_enable(struct device *dev,
				    ...
				    const char *buf, size_t size)
{
    ...
    {
    	user_usb_port_enable = 0;
		usb_port_pullup_disable();
    }
	else
	{
		user_usb_port_enable = 1;
		usb_port_pullup_enable();
//		android_usb_reset();
	}
...
/* hyoill.leem 2012.01.16 usb port enable / disable */
#ifdef CONFIG_LGE_USB_PORT_DISABLE
static DEVICE_ATTR(usb_port_enable, S_IRUGO | S_IWUSR, read_usb_port_enable, write_usb_port_enable);
#endif
/* hyoill.leem 2012.01.16 usb port enable / disable */
...
/* hyoill.leem 2012.01.16 usb port enable / disable */
#ifdef CONFIG_LGE_USB_PORT_DISABLE
	retval = device_create_file(&ui->gadget.dev, &dev_attr_usb_port_enable);
	if (retval != 0)
		dev_err(&ui->pdev->dev,
			"failed to create sysfs entry(usb_port_enable):"
...
/* hyoill.leem 2012.01.16 usb port enable / disable */
#ifdef CONFIG_LGE_USB_PORT_DISABLE
	device_remove_file(&dev->gadget.dev, &dev_attr_usb_port_enable);
#endif
/* hyoill.leem 2012.01.16 usb port enable / disable */
