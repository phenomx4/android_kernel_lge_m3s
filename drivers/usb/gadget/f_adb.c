...
#if 1 // LG_FW_SPG_PORT
	printk(KERN_INFO "[ADB_enable_open] pid -> %x\n",pid);

	if(pid == 0x632F ){
		android_set_pid("61A6",NULL);
		printk(KERN_INFO "[ADB_enable_open] pid -> 61A6\n");
	}
#endif	
/* 2011.05.13 jaeho.cho@lge.com generate ADB USB uevent for gingerbread*/
#ifdef CONFIG_USB_SUPPORT_LGE_ANDROID_AUTORUN
...
#if 1 // LG_FW_SPG_PORT
	printk(KERN_INFO "[ADB_enable_release] pid -> %x\n",pid);
	if(pid == 0x61A6 ){
		android_set_pid("632F",NULL);
		printk(KERN_INFO "[ADB_enable_release] pid -> 632F\n");
	}
#endif
