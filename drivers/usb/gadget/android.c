...
const u16 lg_default_pid	= 0x6310; //hyoill.leem pid change
#define lg_default_pid_string "6310"
#endif
#else
...
#if 1 // LG_FW_SPG_PORT
const u16 lg_ums_pid		= 0x632F;
#define lg_ums_pid_string "632F"
#else
...
#if 1//def LG_FW_SPG_PORT
extern struct android_usb_platform_data android_usb_pdata_spg_port;
extern int msm_get_SPG_PORT_mode(void);
int lg_spg_port_mode = 0;
#endif
...
#elif defined(CONFIG_LGE_USB_GADGET_NDIS_UNITED_DRIVER_NET_MODE)
const u16 lg_ndis_pid = 0x6310; //hyoill.leem pid changed
#define lg_ndis_pid_string "6310"
#endif
...
#endif
#if 1//def LG_FW_SPG_PORT
const u16 lg_adb_ums_pid = 0x61A6;
#define lg_adb_ums_pid_string "61A6"
#endif
...
/* 2011.07.04 jaeho.cho@lge.com for supporting 2 luns */
			 sprintf(serial_number, "%s", "LGANDROIDVM696");
#ifdef CONFIG_LGE_USB_GADGET_F_DIAG_RPC
#else
            msm_hsusb_is_serial_num_null(1); 
#endif
...
#if 1//def LG_FW_SPG_PORT
	else if(product_id == lg_adb_ums_pid)
	{
		device_desc.bDeviceClass		 = USB_CLASS_MISC;
		device_desc.bDeviceSubClass 	 = 0x02;
		device_desc.bDeviceProtocol 	 = 0x01;	
...
#undef CONFIG_LGE_USB_DIAG_DISABLE /* virgin not using */
#if defined(CONFIG_LGE_USB_DIAG_DISABLE) || defined(CONFIG_LGE_USB_PORT_DISABLE)
void android_usb_reset(void)
{
	usb_composite_force_reset(_android_dev->cdev);
}
EXPORT_SYMBOL(android_usb_reset);
#endif
/* 2011.12.07 jaeho.cho@lge.com diag enable/disable*/
...
void android_get_serial_number(char *snum)
{
	snprintf(snum, MAX_SERIAL_LEN, serial_number);
}
...
static int __init android_probe(struct platform_device *pdev)
{
...
#endif
	printk("hyoill.leem calbe type = %d\n",cable);
	...
	// END:0010739 [yk.kim@lge.com] 2010-11-11
#if 1//def LG_FW_SPG_PORT
	else if(!lg_spg_port_mode)
	{
		pdev->dev.platform_data = &android_usb_pdata_spg_port;
		pdata = pdev->dev.platform_data;
		pr_info("[%s] user set product id - spg port lock begin\n", __func__);	
	}
#endif
