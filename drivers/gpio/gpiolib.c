...
int gpio_request(unsigned gpio, const char *label)
{
	...
	if (chip == NULL)
	{
		printk("<choi> LG_FW : %s, chip is null\n",__func__);
		goto done;
	}
	
	if (!try_module_get(chip->owner))
	{
		printk("<choi> LG_FW : %s, try_module_get\n",__func__);
		goto done;
	}
	...
	if (test_and_set_bit(FLAG_REQUESTED, &desc->flags) == 0) {
		...
		printk("<choi> LG_FW : %s, (test_and_set_bit(FLAG_REQUESTED, &desc->flags) == 0)\n",__func__);
		goto done;
	}
	...
	if (chip->request) {
		...
		if (status < 0) {
			printk("<choi> LG_FW : %s, (status < 0) %d\n",__func__, status);
